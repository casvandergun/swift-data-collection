...
- Deprecated `CollectionDispatchWait`. Waiting belongs at the call site, not on the collection: a collection has both latency-sensitive and interactive callers, so one static setting is always wrong for one of them. The next major version makes every mutation return once queued and removes the configuration; callers needing the outcome await `wait()`.
